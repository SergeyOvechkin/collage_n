...
По оси "xy" для искажения используется библиотека **imgwarp-js**, добавление точек деформации производится Ctrl+Click, удаление Shift+Click.
Более детальную информацию о работе библиотеки  **imgwarp-js** можно почитать по ссылке https://chenxing.name/fun/imgwarp-js/
